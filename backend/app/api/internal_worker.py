...
@router.post("/resume-judge-callback")
async def resume_judge_callback(
    payload: ResumeJudgeCallbackPayload,
    _: Annotated[None, Depends(verify_worker_secret)],
    service: Annotated[ApplicationService, Depends(get_application_service)],
) -> dict[str, str]:
    try:
        await service.handle_resume_judge_callback(payload)
    except LookupError as error:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(error)) from error
    except PermissionError as error:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=str(error)) from error
...
@router.post("/keyword-extraction-callback")
async def keyword_extraction_callback(
    payload: KeywordExtractionCallbackPayload,
    _: Annotated[None, Depends(verify_worker_secret)],
    service: Annotated[ApplicationService, Depends(get_application_service)],
) -> dict[str, str]:
    try:
        await service.handle_keyword_extraction_callback(payload)
    except LookupError as error:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(error)) from error
    except PermissionError as error:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=str(error)) from error
