...
The existing dead-code, duplication, and complexity findings were not auto-fixed in this security branch because they are established maintainability debt, not demonstrated vulnerabilities, and broad refactors would add regression risk to the isolation rollout.
...
4. Fallow's existing `ApplicationDetailPage` complexity, dead exports, unused overflow-menu file, and 7.19% duplication should be handled in a separate frontend maintainability task with focused UI regression coverage.
