...
| B5-T46 | Add admin model/reasoning dropdowns and dashboard request quota display | BE/FE/AI/Docs | DONE | 2026-05-24 00:20:00 EDT | Added curated OpenRouter model options for Gemini 3 Flash, GPT 5.4 Mini, and Gemini 3.5 Flash with model-aware reasoning tiers, stored primary/fallback reasoning per subscription tier, passed tier reasoning through worker generation and repair, exposed monthly quota status in session bootstrap, showed dashboard requests remaining, and returned sanitized quota-exhausted errors. |
