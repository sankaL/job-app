...
| `applications.job_keywords` | Object with `status`, `source_hash`, optional `job_id`, optional `model_used`, optional `message`, `updated_at`, optional `extracted_at`, and ordered `keywords` array of exact phrase objects, for example `{"status": "succeeded", "source_hash": "8f5f5b3c...", "model_used": "openai/gpt-5.4-mini", "extracted_at": "2026-06-15T12:10:00+00:00", "updated_at": "2026-06-15T12:10:00+00:00", "keywords": [{"text": "React Native", "source": "extracted"}, {"text": "CI/CD pipelines", "source": "manual", "added_at": "2026-06-17T12:10:00+00:00"}]}` | Stores latest keyword extraction lifecycle state plus user-added manual keywords. Status values are `queued`, `running`, `succeeded`, `failed`, or `unavailable`. Extracted keywords must be exact phrases present in the current `job_description`; manual keywords do not need to appear in the job description. Optional `message` is user-visible sanitized text capped at 240 characters. Coverage is computed at read time from draft Markdown and is warn-only. |
...
| `resume_drafts.generation_params` subscription and operation fields | Optional generated-draft metadata with `subscription_tier`, `quota_period_start`, `model_used`, and optional `keyword_optimization`, for example `{"subscription_tier": "basic", "quota_period_start": "2026-05-01", "model_used": "openai/gpt-5.4-mini", "keyword_optimization": {"enabled": true, "target_keywords": ["Kubernetes"], "preserve_keywords": ["React Native"], "starting_match": {"matched_count": 1, "total_count": 2}}}` | Internal queued model, reasoning override fields, base-resume snapshot content, and current-draft snapshot content must not be persisted in this JSON. Existing non-content base-resume metadata may remain for compare/judge freshness compatibility. |
...
| `job_keywords` | `jsonb` | Yes | `null` | See JSON contract above. Stores the latest ATS keyword extraction state and any user-added manual keywords for the application. |
